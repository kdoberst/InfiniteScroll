...
describe('InfiniteScrollFeed', () => {
  ...
  it('should use singular form for single item', () => {
    render(<InfiniteScrollFeed {...defaultProps} items={['Item 1']} ariaFeedLabel="posts feed, 1 post loaded" />);
    const feed = screen.getByRole('feed');
    expect(feed).toHaveAttribute('aria-label');
  ...
  it('should include "all posts loaded" in aria-label when endOfData is true', () => {
    render(<InfiniteScrollFeed {...defaultProps} endOfData={true} ariaFeedLabel="posts feed, 3 posts loaded, all posts loaded" />);
    const feed = screen.getByRole('feed');
    expect(feed).toHaveAttribute('aria-label');
